...
// https://github.com/baohaojun/system-config/blob/master/gcode/enum-share/shares/Shares.cs
// http://www.pinvoke.net/default.aspx/netapi32/netshareenum.html
// https://stackoverflow.com/questions/8918477/how-can-i-enumerate-network-shares-and-web-folders-in-c
...
// https://stackoverflow.com/questions/2091126/enumerating-network-shares-with-c-sharp
// http://www.codeproject.com/Articles/2939/Network-Shares-and-UNC-paths
// https://stackoverflow.com/questions/3567063/get-a-list-of-all-unc-shared-folders-on-a-local-network-server
// https://danv74.wordpress.com/2011/01/11/list-network-shares-in-c-using-wmi/
...
namespace Trinet.Networking
{
    ...
    public class Share
    {
        ...
        public override string ToString()
        {
            if (null == _server || 0 == _server.Length)
            {
                return string.Format(@"\\{0}\{1}", System.Environment.MachineName, _netName);
            }
            else
    ...
    /// </summary>
    public class ShareCollection : System.Collections.ReadOnlyCollectionBase
    {
        ...
        {
            get { return (System.PlatformID.Win32NT == System.Environment.OSVersion.Platform); }
        }
        ...
        protected static bool IsW2KUp
        {
            get
            {
                ...
                if (System.PlatformID.Win32NT == os.Platform && os.Version.Major >= 5)
                    return true;
                else
                    return false;
            }
        ...
        [DllImport("mpr", CharSet = CharSet.Auto)]
        protected static extern int WNetGetUniversalName(string lpLocalPath,
            int dwInfoLevel, System.IntPtr lpBuffer, ref int lpBufferSize);

        /// <summary>Enumerate shares (NT)</summary>
        [DllImport("netapi32", CharSet = CharSet.Unicode)]
        protected static extern int NetShareEnum(string lpServerName, int dwLevel,
            out System.IntPtr lpBuffer, int dwPrefMaxLen, out int entriesRead,
        ...
        protected static extern int NetShareEnum(
            ...
            System.IntPtr lpBuffer, ushort cbBuffer, out ushort entriesRead,
            out ushort totalEntries);

        /// <summary>Free the buffer (NT)</summary>
        [DllImport("netapi32")]
        protected static extern int NetApiBufferFree(System.IntPtr lpBuffer);
        ...
        protected static void EnumerateSharesNT(string server, ShareCollection shares)
        {
            ...
            System.IntPtr pBuffer = System.IntPtr.Zero;
            ...
            try
            {
                ...
                {
                    System.Type t = (2 == level) ? typeof(SHARE_INFO_2) : typeof(SHARE_INFO_1);
                    int offset = Marshal.SizeOf(t);

                    for (long i = 0, lpItem = pBuffer.ToInt64(); i < entriesRead; i++, lpItem += offset)
                    {
                        System.IntPtr pItem = new System.IntPtr(lpItem);
                        if (1 == level)
                        {
        ...
        public static string PathToUnc(string fileName)
        {
            ...
            {
                System.IntPtr pBuffer = Marshal.AllocHGlobal(bufferSize); ;
                try
                {
                    nRet = WNetGetUniversalName(
                        fileName, UNIVERSAL_NAME_INFO_LEVEL,
            ...
            switch (nRet)
            {
                ...
                default:
                    System.Console.WriteLine("Unknown return value: {0}", nRet);
                    return string.Empty;
            }
        }
